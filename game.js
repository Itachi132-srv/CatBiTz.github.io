...
function init(){
 ...
 for(let i=-250;i<=250;i+=40){
   ...
   let roadH=roadV.clone();
   roadH.rotation.z=Math.PI/2; roadH.position.set(0,0.01,i);
   scene.add(roadH);
 }
 ...
 loader.load('car.glb', function(gltf){
    car=gltf.scene; car.scale.set(0.5,0.5,0.5);
    car.position.set(0,0,0); car.traverse(c=>{if(c.isMesh)c.castShadow=true;});
    scene.add(car);
 });
